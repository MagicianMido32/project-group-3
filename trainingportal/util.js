...
exports.getConfig = () => {
	if (!config) {
		...
		if (!fs.existsSync(configPath)) {
			console.warn(
				`WARNING: Config file not found at ${configPath}. Trying default file.`
			);
...
exports.hasKey = function () {
	var hasKey =
		typeof process.env.CHALLENGE_KEY !== 'undefined' &&
		process.env.CHALLENGE_KEY != null;
	return hasKey;
};
...
exports.log = function (message, user, req) {
	...
	if (
		!exports.isNullOrUndefined(user) &&
		!exports.isNullOrUndefined(user.givenName) &&
		!exports.isNullOrUndefined(user.familyName)
	) {
	...
	if (
		!exports.isNullOrUndefined(req) &&
		!exports.isNullOrUndefined(req.originalUrl)
	) {
