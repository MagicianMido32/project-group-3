...
exports.getPromise = function (dbFunc, params) {
	let promise = new Promise((resolve, reject) => {
		...
		switch (count) {
			case 0:
				dbFunc(reject, resolve);
				break;
			case 1:
				dbFunc(params[0], reject, resolve);
				break;
			case 2:
				dbFunc(params[0], params[1], reject, resolve);
				break;
		}
	});
	return promise;
...
exports.init = async () => {
	...
	var setupNeeded =
		(MYSQL_CONFIG === null && result.length > 0 && result[0].count === 0) ||
		(MYSQL_CONFIG !== null && error !== null);
	...
	if (setupNeeded) {
		...
		if (version < SCHEMA_VERSION) {
			...
			for (let ver = version + 1; ver <= SCHEMA_VERSION; ver++) {
				...
				var maintenanceScript = fs.readFileSync(
					path.join(__dirname, maintenanceFilePath),
					'utf8'
				);
				...
				if (ver === 4) {
					...
					let users = await con.queryPromise(
						'SELECT * FROM users WHERE level > 6'
					);
...
exports.insertUser = function (user, errCb, doneCb) {
	var con = getConn();
	var sql =
		'INSERT INTO users (id, accountId, teamId, familyName, givenName, role) VALUES (null, ?, ?, ?, ?, ?)';

	con.query(
		sql,
		[user.accountId, user.teamId, user.familyName, user.givenName, user.role],
		function (err, result) {
...
exports.updateUser = function (user, errCb, doneCb) {
	var con = getConn();
	var sql =
		'UPDATE users SET accountId = ?, teamId = ?, familyName = ?, givenName = ? WHERE id = ?';
	con.query(
		sql,
		[user.accountId, user.teamId, user.familyName, user.givenName, user.id],
		function (err, result) {
			if (err) handleErr(errCb, err);
			else handleDone(doneCb, result);
		}
...
exports.insertTeam = function (user, team, errCb, doneCb) {
	...
	async.waterfall(
		[
			...
			function (result, cb) {
				exports.getTeamWithMembersByName(
					team.name,
					function (err) {
						cb(err);
					},
...
exports.updateTeam = function (user, team, errCb, doneCb) {
	var con = getConn();
	var sql =
		'UPDATE teams SET name = ?, description = ? WHERE id = ? and ownerId = ?';
	con.query(
		sql,
		[team.name, team.description, team.id, user.id],
		function (err, result) {
			if (err) handleErr(errCb, err);
			else handleDone(doneCb, result);
		}
...
exports.getTeamMembersByBadges = async (teamId, days) => {
	let con = getConn();
	let sql =
		'SELECT badges.moduleId, badges.timestamp, users.givenName, users.familyName FROM users LEFT JOIN badges on badges.userId=users.id';
	let args = [];
...
exports.getAllUsersForBadge = async (moduleId) => {
	let con = getConn();
	let sql =
		'SELECT badges.moduleId, users.givenName, users.familyName FROM users INNER JOIN badges on badges.userId=users.id WHERE badges.moduleId = ? ' +
...
exports.insertChallengeEntry = function (userId, challengeId, errCb, doneCb) {
	...
	con.query(sql, [userId, challengeId], function (err, result) {
		if (err) handleErr(errCb, err);
			...
			var sql =
				'INSERT INTO challengeEntries (id, userId, challengeId, timestamp) VALUES (null, ?, ?, ?)';
...
exports.fetchActivity = function (query, limit, errCb, doneCb) {
	...
	if (!util.isNullOrEmpty(query)) {
		...
		sql =
			'SELECT challengeEntries.challengeId, challengeEntries.timestamp, users.givenName, users.familyName, users.teamId ' +
			' FROM challengeEntries INNER JOIN users on users.id=challengeEntries.userId ' +
			'WHERE ' +
			concat +
			' LIKE ? order by challengeEntries.id desc LIMIT ?';
		args = [query, limit];
	} else {
		sql =
			'SELECT challengeEntries.challengeId, challengeEntries.timestamp, users.givenName, users.familyName, users.teamId ' +
...
exports.getChallengeStats = function (errCb, doneCb) {
	var con = getConn();
	var sql =
		'SELECT challengeId,count(*) as achieved from challengeEntries group by challengeId order by achieved asc;';
	var args = [];
	con.query(sql, args, function (err, result) {
		if (err) handleErr(errCb, err);
		else {
...
exports.getModuleStats = async () => {
	var con = getConn();
	var sql =
		'SELECT badges.moduleId, count(*) as playerCount FROM badges group by badges.moduleId';
	var args = [];
...
exports.getTeamStats = async (limit) => {
	...
	var sql =
		'SELECT teams.id as id, teams.name as teamName, count(users.id) as playerCount from teams INNER JOIN users on users.teamId=teams.id group by teams.id order by playerCount desc';
	var args = [];
