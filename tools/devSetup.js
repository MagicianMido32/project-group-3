...
async function setup() {
	try {
		...
		util.log(
			"Registering local user 'dojouser' with password: 'SecureCodingDojo'"
		);
		//cleanup the account if it exists
		await db
			.getConn()
			.queryPromise("DELETE FROM users WHERE accountId = 'Local_dojouser'");
		//create the dev account
		let dojoUserInfo = {
			accountId: 'Local_dojouser',
			familyName: 'User',
			givenName: 'Dojo',
			role: 'Student',
		};
		...
		await challengeUtil.passChallenges('securityCodeReviewMaster', user, [
			'codereview101_indirectReferences',
		]);
