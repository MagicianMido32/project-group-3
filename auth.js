...
let registerLocalUser = function (req, res) {
	...
	if (
		util.isNullOrUndefined(role) ||
		!roles.includes(role) ||
		validator.isAlphanumeric(role, 'en-US') === false
	) {
		return util.apiResponse(req, res, 400, 'Invalid role.');
	}
...
let verifyLocalUserRole = function (username, form_role) {
	if (localUsers === null) {
		util.log('Local authentication is not configured');
		return null;
	}
...
let getLocalStrategy = function () {
	return new LocalStrategy(
		...
		(req, username, password, cb) => {
			var myRole = verifyLocalUserRole(username, req.body.UserRole);
			// let rrolee = req.body.UserRole;
			var user = verifyLocalUserPassword(username, password);
			if (user !== null && myRole !== null) {
...
let getPassport = function () {
	...
	if ('samlCert' in config) {
		...
		if (config.samlLogProviderMetadata) {
			console.log(
				samlStrategy.generateServiceProviderMetadata(samlProviderCert)
			);
		}
