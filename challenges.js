...
function init() {
	...
	for (let moduleId in modules) {
		...
		for (let level of moduleDefinitions) {
			...
			for (let challenge of level.challenges) {
				...
				descriptions[challenge.id] = path.join(
					modulePath,
	...
	if (util.isNullOrUndefined(process.env.CHALLENGE_MASTER_SALT)) {
		util.log(
			'WARNING. CHALLENGE_MASTER_SALT not set. Challenges may be bypassed.'
		);
	} else {
...
exports.getChallengeNames = function () {
	return challengeNames;
};
exports.getChallengeDefinitions = function () {
...
exports.getUserLevelForModule = async (user, moduleId) => {
	let moduleDefinitions = getDefinifionsForModule(moduleId);
	let passedChallenges = await db.getPromise(
...
exports.getPermittedChallengesForUser = async (user, moduleId) => {
	...
	var permittedLevel =
		(await exports.getUserLevelForModule(user, moduleId)) + 1;
	...
	for (let level of moduleDefinitions) {
		for (let challenge of level.challenges) {
			...
			if (!util.isNullOrUndefined(config.playLinks)) {
				...
				if (
					!util.isNullOrUndefined(description) &&
					description.indexOf(modulePath) === -1
				) {
...
exports.getSolution = function (challengeId) {
	...
	if (!util.isNullOrUndefined(solution)) {
		var solutionMarkDown = fs.readFileSync(
			path.join(__dirname, solution),
			'utf8'
		);
...
exports.getBadgeCode = (badge, user) => {
	...
	if (
		util.isNullOrUndefined(module) ||
		util.isNullOrUndefined(module.badgeInfo)
	)
		return null;
...
module.exports.badgrCall = function (badgrInfo, user) {
	if (
		!util.isNullOrUndefined(badgrInfo) &&
		!util.isNullOrUndefined(config.encBadgrToken)
	) {
		if (user.email === null) {
			...
			badgrInfo.narrative +=
				' Awarded to ' + user.givenName + ' ' + user.familyName + '.';
			...
			try {
				...
				var postReq = https.request(postOptions, function (res) {
					if (
						res !== null &&
						!util.isNullOrUndefined(res.statusCode) &&
						res.statusCode === 201
					) {
...
module.exports.apiChallengeCode = async (req) => {
	if (
		util.isNullOrUndefined(req.body.challengeId) ||
	...
	if (
		util.isNullOrUndefined(challengeCode) ||
		validator.isBase64(challengeCode) == false
	) {
	...
	var availableChallenges = await module.exports.getPermittedChallengesForUser(
		req.user,
		moduleId
	);
	...
	let ms = '';
	if (
		util.isNullOrUndefined(modules[moduleId].skipMasterSalt) ||
		modules[moduleId].skipMasterSalt === false
	) {
		ms = masterSalt;
	}
	...
	var verificationHashB64 = crypto
		.createHash('sha256')
		.update(challengeId + req.user.codeSalt + ms)
		.digest('base64')
		.substr(0, 10);
	var verificationHashHex = crypto
		.createHash('sha256')
		.update(challengeId + req.user.codeSalt + ms)
		.digest('hex')
		.substr(0, 10);
	if (
		challengeCode.indexOf(verificationHashB64) !== 0 &&
		challengeCode.indexOf(verificationHashHex) !== 0
	) {
		throw Error('invalidCode');
	}
	//success update challenge
	curChallengeObj.moduleId = moduleId;
	return module.exports.insertChallengeEntry(
		req.user,
		curChallengeObj,
		moduleId
	);
 ...
 */
module.exports.insertChallengeEntry = async (
	user,
	curChallengeObj,
	moduleId
) => {
	...
	let isModuleComplete = await module.exports.verifyModuleCompletion(
		user,
		moduleId
	);
	...
	if (isModuleComplete) {
		util.log(
			`User has solved the challenge ${curChallengeObj.name} and completed the module!`,
			user
		);
		...
		return {
			message:
				'Congratulations you solved the challenge and completed the module! You can now get your badge of completion.',
