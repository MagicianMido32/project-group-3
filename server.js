...
app.use(
	'/public/angular-route',
	express.static(path.join(__dirname, 'node_modules/angular-route'))
);
app.use(
...
app.use(
	'/public/open-iconic',
	express.static(path.join(__dirname, 'node_modules/open-iconic'))
);
app.use(
	'/public/highlightjs',
	express.static(path.join(__dirname, 'node_modules/highlightjs'))
);
app.use(
	'/public/canvas-confetti',
	express.static(path.join(__dirname, 'node_modules/canvas-confetti'))
);
...
app.get('/public/providers', (req, res) => {
	var providers = [];
	if ('googleClientId' in config)
		providers.push({ name: 'Google', url: '/public/provider/google' });
	if ('slackClientId' in config) {
		var slackLoginDisplayName = 'Slack';
		if ('slackLoginName' in config)
			slackLoginDisplayName = config.slackLoginName;
		providers.push({
			name: slackLoginDisplayName,
			url: '/public/provider/slack',
		});
	}
	if ('samlCert' in config)
		providers.push({ name: 'ADFS SAML', url: '/public/provider/saml' });
	if ('localUsersPath' in config)
		providers.push({ name: 'Local', url: '/public/provider/local' });
	if ('ldapServer' in config)
		providers.push({ name: 'LDAP', url: '/public/provider/ldap' });
...
app.get('/public/badge/:code', async (req, res) => {
	...
	let imgSrc = `${config.dojoUrl}/public/badge/${encodeURIComponent(
		code
	)}/image.png`;
	let html = badgeHtml;
...
app.get('/challenges/:moduleId', async (req, res) => {
	...
	if (
		util.isNullOrUndefined(moduleId) ||
		validator.isAlphanumeric(moduleId) == false
	) {
	...
	if (!allowed) {
		return util.apiResponse(
			...
			res,
			403,
			'Requested module id is not available.'
		);
	}

	var returnChallenges = await challenges.getChallengeDefinitionsForUser(
		req.user,
		moduleId
	);
	var response = {
...
app.get('/challenges/:moduleId/level', async (req, res) => {
	...
	let userLevelForModule = await challenges.getUserLevelForModule(
		req.user,
		moduleId
	);
...
app.get('/challenges/solutions/:challengeId', (req, res) => {
	...
	if (
		util.isNullOrUndefined(challengeId) ||
		util.isAlphanumericOrUnderscore(challengeId) === false
	) {
...
app.get('/api/user', (req, res) => {
	db.fetchChallengeEntriesForUser(
		req.user,
		function () {
			res.send(req.user);
		},
...
app.post('/api/user/team', (req, res) => {
	...
	if (req.user == null || util.isNullOrUndefined(req.user.id)) {
		return util.apiResponse(
			...
			res,
			500,
			"Something's wrong with your session. Logout and log back in."
		);
	}
		...
		function (team) {
			if (team != null) {
				...
				util.apiResponse(
					...
					res,
					200,
					'Team does not exist. Refresh your page.'
				);
			}
		}
	);
...
app.post('/api/user/challengeCode', async (req, res) => {
	try {
		...
		switch (err.message) {
			case 'invalidRequest':
				util.apiResponse(req, res, 400, 'Invalid request.');
				break;
			case 'invalidCode':
				util.apiResponse(req, res, 400, 'Invalid challenge code.');
				break;
			case 'invalidChallengeId':
				util.apiResponse(req, res, 400, 'Invalid challenge id.');
				break;
			case 'invalidModuleId':
				util.apiResponse(req, res, 400, 'Invalid module id.');
				break;
			case 'challengeNotAvailable':
				util.apiResponse(
					...
					res,
					404,
					'Challenge not found for the current user level'
				);
				break;
			case 'challengeSecretNotFound':
				util.apiResponse(req, res, 404, 'Challenge secret not found.');
				break;
			case 'codeSaveError':
				util.apiResponse(req, res, 500, 'Unable to save code.');
				break;
			case 'levelUpError':
				util.apiResponse(
					req,
					res,
					500,
					'Unable to check level up. Please try again.'
				);
				break;
			default:
...
app.get('/api/teams/:teamId/badges', async (req, res) => {
	...
	if (
		util.isNullOrUndefined(teamId) ||
		(validator.isAlphanumeric(teamId) === false && teamId !== '*')
	) {
		return util.apiResponse(req, res, 400, 'Invalid team id.');
	}
...
app.post('/api/teams', auth.ensureApiAuth, (req, res) => {
	...
	if (
		util.isNullOrUndefined(teamName) ||
		validator.matches(teamName, /^[a-z0-9\s_'\-]+$/i) == false
	) {
		return util.apiResponse(
			req,
			res,
			400,
			'Team name must be alphanumeric or name punctuation.'
		);
	}
			...
			db.getTeamWithMembersByName(
				...
				function () {
					util.apiResponse(
						...
						res,
						500,
						'An error occured fetching the newly created team, Check the logs.'
					);
				},
				function (team) {
