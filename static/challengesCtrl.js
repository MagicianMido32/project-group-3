	...
	$scope.loadChallenges = function () {
		$http.get(`/challenges/${$scope.moduleId}`).then(function (response) {
			if (
				...
				$scope.changeViewSolution = false;

				$http
					.get('/api/getviewSolutions', window.getAjaxOpts())
					.then(function (response) {
						if (
							...
							let progress = response.data.result;
							$scope.changeViewSolution = true;

							$scope.moduleChallengeDefinitions.forEach((element) => {
								element.challenges.forEach((challenge) => {
									for (const ch in progress) {
										if (ch === challenge.id) {
											challenge.answer = progress[ch];
										}
									}
								});
							});
					...
					.catch((err) => ($scope.changeViewSolution = false));
