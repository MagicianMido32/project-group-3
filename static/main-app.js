...
app.controller('mainCtrl', [
	...
	'$location',
	'dataSvc',
	function ($scope, $http, $location, dataSvc) {
		...
		$scope.activityHeartBeat = function () {
			$http.get('/api/activity/heartbeat', window.getAjaxOpts()).then(
				function (response) {
					if (
						response != null &&
						response.data != null &&
		...
		$scope.fetchActivity = function () {
			...
			$http
				.get('/api/activity?query=' + filter, window.getAjaxOpts())
		...
		$scope.hasCodeBlocks = function (challenge) {
			return (
				typeof challenge.codeBlockIds !== 'undefined' &&
				challenge.codeBlockIds != null &&
				challenge.codeBlockIds.length > 0
			);
		};
		...
		$scope.deleteOwnedTeam = function () {
			if (confirm('Are you sure?')) {
				$http.delete('/api/user/team', window.getAjaxOpts()).then(
					function (response) {
						if (response != null && response.data != null) {
		...
		$scope.fetchTeams = function () {
			$http.get('/api/teams', window.getAjaxOpts()).then(function (response) {
				if (response != null && response.data != null) {
					$scope.teamNames = {};
					...
					$http
						.get('/api/users', window.getAjaxOpts())
						.then(function (response) {
							if (response != null && response.data != null) {
								for (let team of teamList) {
									if (team.ownerId != null && team.ownerId === $scope.user.id) {
										// the user cannot change their team until they delete their current team
		...
		$scope.onUserTeamChange = function () {
			if (
				$scope.teamList !== null &&
				typeof userTeamListChoice !== 'undefined' &&
				userTeamListChoice.value !== ''
			) {
		...
		$scope.loadData = function () {
			$http.get('/api/user', window.getAjaxOpts()).then(function (response) {
				if (response != null && response.data != null) {
					...
					$http
						.get('static/codeBlocks/codeBlocksDefinitions.json')
						.then(function (response) {
							if (response != null && response.data != null) {
